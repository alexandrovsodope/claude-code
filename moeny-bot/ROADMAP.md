...
- Этап 5b (2026-09-24, backup = первый v5): всплывающее уведомление на каждый тап; на кнопках «✔N» (сколько
  отметились) и «💳 Имя» (кто оплатил за других); клавиатура строится из нового текста (SetVariables);
  до 15 позиций на карточке; понятнее подсказка. Логика: в обоих режимах каждый отмечает, что ел;
  «✅ Платил(а) X» — один заплатил за всё; «🙋 Каждый за себя» — долгов нет, кроме блюд с 💳.
- Этап 5c (2026-09-24): ФИКС «кнопки чека не работают». Причина: telegram:UniversalAPICall с bodyType=mapped_body
  и объектом в body НЕ передаёт параметры (проверено: getChat → «chat_id is empty»), answerCallbackQuery падал,
  Ignore останавливал маршрут → карточка не обновлялась. Теперь assembled_body + body_spec (для всех answerCallbackQuery),
  а у ответа на тап/отмену обработчик Resume (карточка обновится, даже если ответ не прошёл).
  «Закрыть чек» без отметок / не отправителем теперь показывает всплывашку с причиной.
- Этап 5d (2026-09-24): ФИКС «отметки теряются при быстрых нажатиях». Причина: callback несёт текст карточки
  на момент нажатия, а правка карточки приходит через 2–3 с → следующий тап затирал предыдущий. Теперь актуальный
  текст карточки хранится в Make Data Store «ReceiptState» (ключ chat_message), тап читает/пишет его (сценарий
  sequential, гонок нет). Закрытие чека тоже читает из хранилища; запись удаляется при закрытии/отмене.
  Тап теперь ≈ 6 оп. (было 4). Проверено харнессом: 6 тапов с устаревшим текстом → все отметки на месте.
- Этап 5e (2026-09-24): подсказки «➕ Добавить трату» / «🧾 Чек» видит только тот, кто нажал (всплывающее окно
  answerCallbackQuery show_alert), сообщения-подсказки в чат больше не пишутся (маршруты 2005/2310 удалены).
  В группе у бота включён privacy mode, поэтому траты и чеки добавляются ОТВЕТОМ на любое сообщение бота;
  фото-ответ боту = чек. «🤔 Не понял трату» показывается только на ответы боту (болтовню в группе не трогает).
- Этап 5f (2026-09-26): ФИКС «бот умер после 🙋 Каждый за себя». Причина: «💰 Баланс» в группе без долгов —
  агрегатор Make при 0 записей отдаёт [{}], map(...;"u") → [null], итератор падает («Cannot convert undefined or null
  to object»), а сценарий sequential → Make выключает его целиком. Фикс: ключ k в агрегаторах 2062/2102 и
  map(...;"u";"k";"1"). Проверено харнессом на группе без долгов и с долгами. Плюс: закрытие чека только если есть
  запись в ReceiptState (нет двойного закрытия), и Gemini дробит 2–6 одинаковых штук на отдельные строки
  («🥐 Круассан 1/2/3»). ВАЖНО: любая необработанная ошибка выключает весь бот (sequential) — новые итераторы
  по массивам из агрегаторов всегда фильтровать через k.
